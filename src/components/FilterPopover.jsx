import {
  ...
  //Component which renders each status in filter popup
  const StatusItem = ({ status, setColumnFilters, isActive }) => (
    <Flex //common stylings
      align="center"
      cursor="pointer"
      borderRadius={5}
      fontWeight="bold"
      ...
      //updating the table columnFilters state when it clicked
      onClick={() =>
        setColumnFilters((prev) => {
            //check for any status filters are present, otherwise concat new filter object
  ...
  //if already statuas filter was active or previours filters are present 
  //then clicked again it should be removed
          return prev.map((f) =>
            f.id === "status"
              ? {
                  ...f,
  ...
  const FilterPopover = ({ columnFilters, setColumnFilters }) => {
    //grapping all active status filters
    const filterStatuses =
      columnFilters.find((f) => f.id === "status")?.value || [];
  
    return (
        //Creating the pop up for filter based on status
      <Popover isLazy>
        <PopoverTrigger>
          <Button
            size="sm"
